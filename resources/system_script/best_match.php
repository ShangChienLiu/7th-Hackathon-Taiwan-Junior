<?php
    ...
    */
    ...
    $be_volunteer = $sql -> prepare('SELECT * FROM `be_volunteer` WHERE 1');
    $be_volunteer -> execute();

    $find_volunteer = $sql -> prepare('SELECT * FROM `find_volunteer` WHERE `progress` = 2');
    ...
    while($row = $be_volunteer->fetch(PDO::FETCH_ASSOC)){
        ...
        */
        ...
        foreach($array_match_period as $item){
            ...
            while($row2 = $find_volunteer_by_id->fetch(PDO::FETCH_ASSOC)){
                // 修正計算方法
                $cal = abs(strtotime($row2['start_date']) - strtotime($target_start_date)) + abs(strtotime($row2['end_date']) - strtotime($target_end_date));
                if($cal > $best_duration){
                    $best_duration = $cal;
                    $best_id = $row2['id'];
                    $last_update_at = $row2['updated_at'];
                    $end_time = date("Y-d-m h:i:s", min(strtotime($row2['end_date']), strtotime($target_end_date)));
                    $start_time = date(min(strtotime($row2['start_date']), strtotime($target_start_date)));
